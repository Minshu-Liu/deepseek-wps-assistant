...
echo.
echo 卸载源流WPS助手前，请先保存并关闭所有 WPS 文档。
echo 继续卸载将自动关闭所有 WPS 进程。
echo.
...
if exist "%ADDON_DIR%" (
    echo [警告] 插件目录仍存在，请尝试右键 uninstall.bat 以管理员身份运行。
) else (
...
echo.
echo 卸载完成，请重新打开 WPS 检查「源流WPS助手」是否消失。
echo.
mshta "javascript:alert('源流WPS助手已卸载完成，请重新打开 WPS 检查选项卡是否消失。');close();"
pause
exit /b 0